...
def add(added):
    added = added or input('Eureka? ')  # input prompt if empty '' string is passed
...
def delete():  # TODO: Instead of just accepting one line, create a while loop accepting to be deleted lines
...
def else_what(string):
    answer = input('Add this as an EUREKA? [y/N]: ')
    if answer == 'y':
        add(string)
        debug_add('Quick added EUREKA')
    else:
        print('\nThen, WTF is dis:', string)
        debug_add("User can't follow instructions")
